...
function Index(props) {
	...
	return (<div className={s.pizza}>
		<img className={s.image} src={props.imageUrl} alt='картинка' />
		<div className={s.title}>{props.name}</div>
		<div className={s.settings}>
			<div className={s.dough}>
				{typeNames.map((item, index) => (<div key={'type' + index}
																							className={classNames(activeType === index ? s.active : null, !props.types.includes(index) ? s.disabled : null)}
																							onClick={() => onSelectType(index)}>{item}
				</div>))}
			</div>
			<div className={s.size}>
				{size.map((item, index) => (<div key={'type' + index}
																				 className={classNames(!props.sizes.includes(item) ? s.disabled : null, activeSize === index ? s.active : null)}
		...
		<div className={s.footer}>
			...
			<div className={classNames(s.addButton)} onClick={() => props.onAddCartItem(cartData)}><img
				src='./img/plusOrange.svg' alt='добавить' />Добавить {counter !== 0 && <span>{counter}</span>}
			</div>
		</div>
	</div>)
}
