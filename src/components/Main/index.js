...
function Index() {
	...
	React.useEffect(() => {
		const queryString = qs.stringify({
			sortBy: sortBy, category: category, page: page
		})
	...
	const subMenu = [{ name: 'популярности' }, { name: 'цене' }, { name: 'алфавиту' }]
	...
	const debounceFilterInput = debounce((e) => dispatch(setSearchFilter(e.target.value)), 400)
	...
	return (<div className={s.main}>
		<div className={s.sorting}>
			<div className={s.sortingLeft}>
				{sortingButton.map((item, index) => (<div key={`Btn${index}`} className={category === index ? s.active : null}
			...
			<div className={s.sortingRight} ref={sortRef}>
				...
				{isSubMenu && <div className={s.subMenu}>
					<div className={s.features}>
						{subMenu.map((item, index) => (<p onClick={() => onActiveSubMenu(index)} key={'subMenuItem' + index}
		...
		<div className={s.pizzas}>
			{isLoaded ? renderItem.map((item, index) => (<Pizza
				key={`${item.name}${index}`}
